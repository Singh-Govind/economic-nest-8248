...
const LeftBox = () => {
  ...
  const getData = async () => {
    await axios.get("/api/products/category").then((res) => {
  ...
  const brandFilterData = async (brandName) => {
    await axios.get(`/api/products/category?brand=${brandName}`).then((res) => {
      setProducts(res.data.data);
    });
  };
  ...
  const filterByPrice = async (el) => {
    ...
    await axios.get(`/api/products/category?price=${price}`).then((res) => {
  ...
  const filterByType = async (type) => {
    await axios.get(`/api/products/category?type=${type}`).then((res) => {
      console.log(res.data);
      setProducts(res.data.data);
    });
  };
  ...
  const filterByPriceRange = async (cmd) => {
    if (cmd == "gt") {
      let price = 8000;
      await axios
        .get(`/api/products/category?price=${price}&cmd=${cmd}`)
        .then((res) => {
          setProducts(res.data);
        });
    } else if (cmd == "lt") {
      ...
      await axios
        .get(`/api/products/category?price=8000&cmd=${cmd}`)
        .then((res) => {
      ...
      await axios
        .get(`/api/products/category?price=5000&cmd=${cmd}`)
        .then((res) => {
          setProducts(res.data);
        });
  };
          ...
          <Box display="flex" gap={2} alignItems="center">
            ...
            <Box>
              ...
              >
                ...
                <option value="gte">Price (high to low)</option>
              </Select>
            </Box>
          </Box>
        </Flex>
